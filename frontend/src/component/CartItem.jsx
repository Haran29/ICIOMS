...
/*const CartItem = ({ cartItem, updateQuantity, removeItem }) => (
  ...
  const CartItem = ({ cartItem, updateQuantity, removeItem }) => {
    const handleUpdateQuantity = (newQuantity) => {
      ...
      if (newQuantity <= 0) {
        toast.error("Quantity cannot be zero or negative.", { position: "top-right" });
        return;
      }
  
      if (newQuantity > availableQuantity) {
        toast.error(`Only ${availableQuantity} ${cartItem.itemId.name} available.`, { position: "top-right" });
        return;
      }
